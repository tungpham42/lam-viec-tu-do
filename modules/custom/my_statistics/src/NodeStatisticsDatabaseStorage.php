...
/**
 * Provides the default database storage backend for my_statistics.
 */
class NodeStatisticsDatabaseStorage implements StatisticsStorageInterface {
  ...
  */
  ...
  /**
   * Constructs the my_statistics storage.
   *
  ...
  public function resetDayCount() {
    $my_statistics_timestamp = $this->state->get('my_statistics.day_timestamp') ?: 0;
    if (($this->getRequestTime() - $my_statistics_timestamp) >= 86400) {
      $this->state->set('my_statistics.day_timestamp', $this->getRequestTime());
      $this->connection->update('my_node_counter')
        ->fields(['daycount' => 0])
        ->execute();
    }
  }
  ...
  public function maxTotalCount() {
    $query = $this->connection->select('my_node_counter', 'nc');
